...
class menu:
    ...
    admin = (
        ...
        "/remind_receipts [YYYY-MM] — напомнить самозанятым о чеках (без аргумента — за все месяцы)\n"
        ...
        "/env_edit <имя> <поле> <значение> — редактировать\n"
        "/env_bind <имя> — привязать чат к окружению\n"
        "/env_unbind — отвязать чат от окружения\n"
        "/env_create <имя> <описание> — создать окружение\n"
        "/env_summarize — извлечь знания из истории чата\n"
        "/perm [tool] — разрешения инструментов\n\n"
        "Пользователи:\n"
        "~ /user_create <описание> — добавить/обновить пользователя\n"
...
class admin:
    ...
    upload_processing = "Обрабатываю выписку (курс {rate} EUR→RUB)..."
