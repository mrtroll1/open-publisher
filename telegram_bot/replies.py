...
class menu:
    ...
    admin = (
        ...
        "/lookup <имя> — информация о контрагенте (автор/редактор/корректор/...)\n\n"
        "Tech:\n"
        ...
        "Сущности:\n"
        "/entity [запрос] — список или поиск сущностей\n"
        "/entity_add <тип> <имя> — добавить сущность\n"
        "/entity_link <имя> <key=value> — привязать внешний ID\n"
        "/entity_note <имя> <текст> — добавить заметку о сущности\n"
        "/sync_entities — синхронизировать контрагентов как сущности\n\n"
        "Окружения:\n"
        "/env [имя] — список или детали окружений\n"
        "/env_edit <имя> <поле> <значение> — редактировать окружение\n"
        "/env_bind <имя> — привязать текущий чат к окружению\n"
        "/env_unbind — отвязать текущий чат от окружения\n"
        "/env_create <имя> <описание> — создать новое окружение\n\n"
        "Пайплайны:\n"
        "/ingest_articles [date_from] [date_to] — загрузить статьи в базу знаний\n"
    ...
    group = (
        ...
        "/support [-e] [-v] <вопрос> — вопрос по техподдержке\n\n"
        "Также можно задать любой вопрос через @-упоминание бота или ответ на его сообщение."
    )
...
class admin:
    ...
    ingest_articles_start = "Загружаю статьи за {date_from} — {date_to}..."
    ingest_articles_no_posts = "Статей за {date_from} — {date_to} не найдено."
    ingest_articles_done = "Загружено {count} записей за {date_from} — {date_to} ({authors} авторов)."
...
class env:
    ...
    created = "Окружение <b>{name}</b> создано."
    unbound = "Чат отвязан от окружения."
