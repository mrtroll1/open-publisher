...
class TestResolveExistingInvoice:
    ...
    @patch("backend.domain.services.invoice_service.prepare_existing_invoice")
    def test_no_invoice_returns_none(self, mock_prep):
        from backend.domain.services.invoice_service import resolve_existing_invoice
...
class TestPrepareNewInvoiceData:
    ...
    @patch("backend.domain.services.invoice_service.resolve_amount")
    @patch("backend.domain.services.invoice_service.fetch_articles")
    @patch("backend.domain.services.invoice_service.read_budget_amounts")
    def test_returns_data_when_amount_exists(self, mock_budget, mock_articles, mock_resolve):
        from backend.domain.services.invoice_service import prepare_new_invoice_data
