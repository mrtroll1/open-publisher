...
- [ ] Each command file has a concrete Controller class extending BaseController
- [ ] Each controller pairs a Preparer with a UseCase via constructor
- [ ] `ROUTES` dict is populated with all routes after `create_brain()` is called
- [ ] `brain.process_command("health", "", "default", "admin")` returns healthcheck results
- [ ] `brain.process("привет", "default", "admin")` returns a conversation reply
- [ ] `brain.process_command("teach", "test fact", "default", "admin")` stores knowledge
- [ ] `brain.process_command("search", "республика", "default", "admin")` returns results
- [ ] Old wiring functions (`create_inbox_service`, etc.) still work
- [ ] All existing tests still pass
- [ ] No circular imports between brain/, commands/, infrastructure/
- [ ] Authorizer correctly resolves environment by chat_id
- [ ] Authorizer correctly resolves user by telegram_user_id
- [ ] Router falls back to "conversation" when no route matches
